/*
  ...
  1. Updates
    - Ensure email templates use correct Supabase placeholders
    - Update confirmation URL format to match expected parameters
    - Add better error handling for email confirmation
...
-- Update email confirmation template to use proper Supabase variables and URL format
...
-- Update password reset template to use proper Supabase variables
UPDATE email_templates 
SET html_content = REPLACE(
    REPLACE(html_content, '{{reset_url}}{{.RedirectTo}}', '{{ .RedirectTo }}'),
    '{{reset_url}}', '{{ .RedirectTo }}'
),
updated_at = now()
WHERE type = 'password_reset';

-- Ensure all templates have proper Supabase variable format
UPDATE email_templates 
SET html_content = REPLACE(
    REPLACE(
        REPLACE(html_content, '{{user_email}}{{.Email}}', '{{ .Email }}'),
